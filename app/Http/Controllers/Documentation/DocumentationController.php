...
class DocumentationController extends Controller
{
    ...
    public function searchStaffProcess (Request $request){
        ...
        if(PersonnelMain::where('file_number','LIKE','%'.$request->username.'%')->orWhere('name','LIKE','%'.$request->username.'%')->get()->count() > 0){
            ...
            $entries = PersonnelMain::where('file_number','LIKE','%'.$request->username.'%')->orWhere('name','LIKE','%'.$request->username.'%')->get();
 
            return back()->with(['data'=> $entries,'username' =>$request->username]);
